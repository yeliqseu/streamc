Introduction
=======
This project implements a kind of packet-level forward-erasure correction (FEC) codes called _streaming codes_, which encodes packets in a convolutional manner to address packet erasures. The codes are also referred to as sliding-window linear codes in literature. The key characteristic of the codes is that the packets at the destination would be reocvered and delivered to upper layers in a _smoothly_ in-order manner, instead of all-together as in conventional block codes which incurs block coding delay. The library can be tested by
...
**Configuration 1**: Send a repair packet after sending every constant-_l_ source packets. 
---
...
> M. Karzand, D. J. Leith, J. Cloud and M. M¨¦dard, "Design of FEC for Low Delay in 5G," in IEEE Journal on Selected Areas in Communications, vol. 35, no. 8, pp. 1783-1793, Aug. 2017.

This configuration can be tested for example by
```shell
$ ./streamcTestIrreg 1000 0 2 0.1 10 0
```
which streams 1000 source packets where the repair packets are inserted every **2** source packets.

**Configuration 2**: Send a repair packet with probability _f_, or a source packet with probability _1-f_. 
---
The most frequent insertion of repair packets of the codes proposed in above papers is _1_. To insert repair packets more often (in case the links are very lossy), streaming codes proposed in the following paper by the author of the library can be used.
